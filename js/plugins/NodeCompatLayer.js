...
// Node.js API 兼容层 v1.0 - Mock 实现（向后兼容）
// 插件版本: v1.0.0
	...
	var NodeCompatLayer = {
		 ...
		 */ createRequireFunction: function createRequireFunction() {
			...
			return function require2(moduleName) {
				...
				console.warn(
					"[NodeCompatLayer] You can register custom modules using: NodeCompatLayer.registerModule('".concat(
						moduleName,
						"', yourModuleExports)",
					),
				);
		 ...
		 */ createFsModule: function createFsModule() {
			return {
				...
				readFileSync: function readFileSync(path) {
					console.warn(
						"[NodeCompatLayer] fs.readFileSync('".concat(path, "') called in browser - returning empty string"),
					);
					return "";
				},
				...
				readdirSync: function readdirSync(path) {
					console.warn(
						"[NodeCompatLayer] fs.readdirSync('".concat(path, "') called in browser - returning empty array"),
					);
					return [];
				},
		 ...
		 */ createNwGuiModule: function createNwGuiModule() {
			...
			MockMenu.prototype.createMacBuiltin = function (appName, _options) {
				console.warn(
					"[NodeCompatLayer] nw.gui.Menu.createMacBuiltin('".concat(appName, "') called in browser - no-op"),
				);
			};
			return {
				Window: {
